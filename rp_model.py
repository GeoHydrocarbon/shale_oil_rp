...
def compute_matrix_moduli(frame: pd.DataFrame, config: dict, EM) -> tuple[np.ndarray, np.ndarray]:
    # 根据矿物体积分数和矿物弹性参数计算等效基质模量。
    mineral_names = list(config["columns"]["minerals"].keys())
    volume_columns = [f"{name}_vol" for name in mineral_names]
    valid = frame["mineral_valid"].to_numpy(dtype=bool)
    ...
    if valid.any():
        ...
        k_values = np.array([config["minerals"][name]["K"] for name in mineral_names], dtype=float)
        g_values = np.array([config["minerals"][name]["G"] for name in mineral_names], dtype=float)
...
def compute_dry_frame(
    ...
    if dry_model == "dem":
        aspect_ratio = float(config["model"]["pore_aspect_ratio"])
        active = valid & (phi >= 0.0) & (phi < 1.0)
        for index in np.where(active)[0]:
            k_path, g_path, t = EM.Berryman_DEM(k_matrix[index], g_matrix[index], 0.0, 0.0, aspect_ratio, phi[index])
            k_dry[index] = float(np.interp(phi[index], t, k_path))
            g_dry[index] = float(np.interp(phi[index], t, g_path))
        return k_dry, g_dry
...
def run_model(frame: pd.DataFrame, config: dict, base_dir: Path) -> tuple[pd.DataFrame, dict[str, int]]:
    ...
    # 第四步：根据孔隙度和孔隙长宽比估算干岩骨架模量。
    k_dry, g_dry = compute_dry_frame(k_matrix, g_matrix, phi, valid, config, EM)
