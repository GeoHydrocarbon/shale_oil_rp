...
from sensitivity_v2 import METRIC_INFO, score_result_frame, validate_metrics
...
def get_fracture_score_weights(config: dict) -> dict[str, float]:
    fracture_cfg = config.get("fracture", {})
    weights = fracture_cfg.get("score_weights", {})
    if not weights:
        return {
            "vp_rmse": 1.0,
            "vs_rmse": 1.0,
...
def compute_fracture_velocities(
    ...
    # 第一步：在当前无裂缝背景上叠加给定裂缝密度和倾角，计算等效 TI 介质速度。
...
def evaluate_fracture_scenario(
    ...
    valid = (
        velocities["fracture_valid"]
        & np.isfinite(velocities["vp_fracture_m_s"])
        & np.isfinite(velocities["vsv_fracture_m_s"])
    )
    ...
    # 第二步：把每一组裂缝场景转换成可比较的误差指标，供后续热图和反演使用。
    return {
...
def scan_fracture_grid(frame: pd.DataFrame, config: dict, base_dir: Path) -> pd.DataFrame:
    ...
    return score_result_frame(result, get_fracture_score_weights(config))


def plot_fracture_grid(grid_frame: pd.DataFrame, config: dict, base_dir: Path) -> Path:
    fracture_cfg = config["fracture"]
    metrics = validate_metrics(
        list(fracture_cfg.get("grid_metrics", ["composite_score", "vp_rmse", "vs_rmse"])),
...
def run_fracture_analysis(frame: pd.DataFrame, config: dict, base_dir: Path) -> dict | None:
    ...
    run_forward_model = bool(fracture_cfg.get("run_forward_model", True))
    run_sensitivity_analysis = bool(fracture_cfg.get("run_sensitivity_analysis", True))
    if not run_forward_model and not run_sensitivity_analysis:
        return None
    ...
    result: dict[str, object] = {
        "summary": {
            "run_forward_model": run_forward_model,
    ...
    if run_forward_model:
        ...
        result["summary"]["reference_density"] = reference_density
        result["summary"]["reference_dip_angle_deg"] = reference_dip_angle_deg
    ...
    if run_sensitivity_analysis:
        ...
        # 第三步：绘制裂缝密度-倾角热图，为后续裂缝密度反演提供先验。
        grid_figure_path = plot_fracture_grid(grid_frame, config, base_dir)

        result["output_table"] = output_table
        result["grid_figure_path"] = grid_figure_path
        result["summary"]["best_fracture_density"] = float(best_row["fracture_density"])
        result["summary"]["best_dip_angle_deg"] = float(best_row["fracture_dip_angle_deg"])
        result["summary"]["grid_rows"] = int(len(grid_frame))
